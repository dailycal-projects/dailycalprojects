...
(function (root) {
  ...
  function equatorial(timeMs) {
    ...
    const x = day - index;
    return {
      ra: polynomial(row, 0, x),
      dec: polynomial(row, 4, x),
      distance: polynomial(row, 8, x),
      gast: polynomial(row, 12, x),
  ...
  function observer(latitude, longitude) {
    ...
    const phi = latitude * RAD; const
      lambda = longitude * RAD;
    const sinPhi = Math.sin(phi); const
      cosPhi = Math.cos(phi);
  ...
  function positionAt(timeMs, site) {
    ...
    const cx = Math.cos(sun.poleX); const
      sx = Math.sin(sun.poleX);
    const cy = Math.cos(sun.poleY); const
      sy = Math.sin(sun.poleY);
    ...
    const bx = -site.y * omegaOverC; const
      by = site.x * omegaOverC;
    const dot = x * bx + y * by;
  ...
  function sunset(year, dayOfYear, latitude, longitude) {
    ...
    let lo = noon; let
      loAltitude = positionAt(lo, site).altitude;
