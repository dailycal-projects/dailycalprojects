...
const Map = () => {
  ...
  const generateMap = (ref, mapData, unis) => {
    ...
    svg
      ...
      .attr("d", path)
      .attr("fill", (d) =>
        mapData.includes(stateNames[d.id]) ? "#176893" : "#cbd4e3"
      )
      .attr("stroke", "#fff")
  ...
  const generateAfterMap = () => {
    const filteredUniversities = pac12Universities.filter(
      (d) => selectedOption === d.conf
    );
      ...
      <div style={{ display: "flex", width: "100%", overflow: "hidden" }}>
        ...
        >
          <h4 style={{ marginBottom: "0px" }}>
            The modern Pac-12 before dissolution
          </h4>
          <svg
            ref={beforeMapRef}
            style={{
        ...
        >
          <h4 style={{ marginBottom: "0px" }}>
            Which Pac-12 universities are now headed to
          </h4>
