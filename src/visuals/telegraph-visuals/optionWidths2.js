...
const roadSections = {
  ...
  "Curb extension":
    "<b>Curb extension</b>: These are extensions of sidewalks into the street. These have many benefits for safety, such as increasing visibility and calming traffic",
  "Bike lane":
    "<b>Bike lane</b>: A lane designated for bikes only. These currently exist on Bancroft",
  "Shared lane":
    "<b>Shared lane</b>: A lane accessible by cars, buses, and bikes. In some cities, these represent recommended cycling lanes, or lanes that should preferrably be bikes-only.",
...
const OptionWidthViz2 = () => {
  ...
  const colorScale = d3.scaleOrdinal(customScheme).domain(keys);
  ...
  useEffect(() => {
    ...
    svg
      ...
      .attr(
        "transform",
        `translate(${config.paddingLeft},${config.h - config.paddingBot})`
      )
      .call(xAxis);
        ...
        (enter) => {
          enter
            ...
            .attr("x", (d, i) => {
              ...
              const offset =
                prevWidths.reduce((sum, width) => sum + width, 0) % 60;
            ...
            .on("mouseover", function mouseAction(event, d) {
              tooltipA.transition().duration(200).style("opacity", 0.9);
              const tooltipContent = `${splitSectionName(d.Section)}: ${
                d.Width
              } feet`;
              ...
              tooltipB.transition().duration(400).style("opacity", 0.9);
            ...
            .on("mouseout", () => {
              tooltipA.transition().duration(500).style("opacity", 0);
              tooltipB.transition().duration(500).style("opacity", 0);
            });
        },
        (update) => {
          update
    ...
    svg
      ...
      .attr(
        "cx",
        isMobile ? config.paddingLeft : config.w - config.paddingRight * 0.5
      )
      .attr("cy", (d, i) =>
        isMobile
          ? config.h -
            0.85 * config.paddingBot +
            (i + 1) * 5 * config.legend_radius
    ...
    svg
      ...
      .attr(
        "x",
        isMobile
          ? config.paddingLeft + config.legend_radius * 1.5
          : config.w - config.paddingRight * 0.5 + config.legend_radius * 2
      )
      .attr("y", (d, i) =>
        isMobile
          ? config.h -
