...
const roadSections = {
  ...
  "Curb extension":
    "Curb extension: These are extensions of sidewalks into the street. These have many benefits for safety, such as increasing visibility and calming traffic",
  "Bike lane":
    "BIke lane: A lane designated for bikes only. These currently exist on Bancroft",
  "Shared lane":
    "Shared lane: A lane accessible by cars, buses, and bikes. In some cities, these represent recommended cycling lanes, or lanes that should preferrably be bikes-only.",
...
const OptionWidthBarChart = () => {
  ...
  useEffect(() => {
    ...
    svg
      ...
      .attr(
        "transform",
        `translate(${config.paddingLeft},${config.h - config.paddingTop})`
      )
      .call(xAxis);
